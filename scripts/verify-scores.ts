...
test('分類は選択計算ではなく定義一覧を持つ', () => {
  ...
  assert.equal(macro.hierarchyOverviews?.[0]?.nodes.at(-1)?.label, '混合型');
  assert.equal(macro.hierarchyOverviews?.[1]?.nodes.at(-1)?.label, '混合型');
  assert.equal(macro.hierarchyOverviews?.[2]?.nodes.at(-1)?.label, '混合型');
  assert.equal(macro.hierarchyOverviews?.[3]?.nodes.at(-1)?.label, '混合型');
  const type0Headings = macro.entries.find((entry) => entry.label === '0-I')?.rows.map((row) => row.heading).join(' ') ?? '';
  assert.match(type0Headings, /Paris/);
  assert.match(type0Headings, /胃/);
  assert.doesNotMatch(type0Headings, /大腸|食道/);
  ...
  assert.equal(macro.entries.find((entry) => entry.label === '0-Ip')?.rows.some((row) => row.heading === '胃'), false);
  assert.equal(macro.entries.find((entry) => entry.label === '0-Isp')?.rows.some((row) => row.heading === '胃'), false);
  assert.equal(macro.entries.find((entry) => entry.label === '0-Is')?.rows.some((row) => row.heading === '胃'), false);
  ...
  assert.doesNotMatch(macro.entries.map((entry) => JSON.stringify(entry)).join(' '), /約 2 mm|英語第3版|未確認|0-I を参照/);
  ...
  assert.match(macro.entries.find((entry) => entry.label === '0-III')?.rows.find((row) => row.heading === '大腸')?.text ?? '', /ほとんど使わないので削除/);
...
test('分類は原著の図を出典付きで持つ', () => {
  ...
  assert.equal(kudo.figures?.[1]?.href, '/figures/pit-pattern-gemini2.svg');
  ...
  assert.match(englishKudo.figures?.[1]?.note ?? '', /Gemini-generated reference schematic/);
...
test('WASP / MESDA-G / Toya / Kikuchi ME-NBI のフローは選択すると診断まで進む', () => {
  ...
  assert.ok(mapLabels(mesda.flow.map).includes('Demarcation line (DL)'));
...
test('臓器ページのサブカテゴリ（フェーズ）にはアイコン画像がある', () => {
  ...
  const { LIST_CLINICAL_PHASE_ORDER } = require('../types/score') as typeof import('../types/score');

  for (const phase of LIST_CLINICAL_PHASE_ORDER) {
    assert.ok(
      existsSync(join(process.cwd(), 'assets/images/list-phase', `${phase}.webp`)),
      `${phase}.webp`,
    );
  }
});
