...
/** ESD/EMR ガイドライン Fig. 2 相当テーブルの行キー */
export type GastricEsdCurabilityRowKey =
  | 'pt1a-ul0'
  | 'pt1a-ul1'
  | 'pt1b-sm1'
  | 'pt1b-sm2';
...
/** 列キー（左=分化型、右=未分化型） */
export type GastricEsdCurabilityColKey =
  | 'diff-le3'
  | 'diff-gt3'
  | 'undiff-le2'
  | 'undiff-gt2';
...
export type GastricEsdCurabilitySizedCellId =
  | 'cell-pt1a-ul0-diff-le3'
  | 'cell-pt1a-ul0-diff-gt3'
  | 'cell-pt1a-ul0-undiff-le2'
  | 'cell-pt1a-ul0-undiff-gt2'
  | 'cell-pt1a-ul1-diff-le3'
  | 'cell-pt1a-ul1-diff-gt3'
  | 'cell-pt1a-ul1-undiff-le2'
  | 'cell-pt1a-ul1-undiff-gt2'
  | 'cell-pt1b-sm1-diff-le3'
  | 'cell-pt1b-sm1-diff-gt3'
  | 'cell-pt1b-sm1-undiff-le2'
  | 'cell-pt1b-sm1-undiff-gt2'
  | 'cell-pt1b-sm2-diff-le3'
  | 'cell-pt1b-sm2-diff-gt3'
  | 'cell-pt1b-sm2-undiff-le2'
  | 'cell-pt1b-sm2-undiff-gt2';
...
export function gastricCurabilityCellId(
  row: GastricEsdCurabilityRowKey,
  col: GastricEsdCurabilityColKey,
): GastricEsdCurabilitySizedCellId {
  return `cell-${row}-${col}` as GastricEsdCurabilitySizedCellId;
}
...
function tumorFactorRow(values: Record<string, number>): GastricEsdCurabilityRowKey | null {
  const { depth, ul } = values;
  if (depth === 0) return ul === 0 ? 'pt1a-ul0' : 'pt1a-ul1';
  if (depth === 1) return 'pt1b-sm1';
  if (depth >= 2) return 'pt1b-sm2';
  return null;
}

function colsForHighlight(
  row: GastricEsdCurabilityRowKey,
  histology: number,
  size?: number,
): GastricEsdCurabilityColKey[] {
  const isDiff = isDifferentiatedDominant(histology);

  if (row === 'pt1a-ul0' && isDiff) {
    return ['diff-le3', 'diff-gt3'];
  }

  if (size === undefined) {
    return isDiff ? ['diff-le3', 'diff-gt3'] : ['undiff-le2', 'undiff-gt2'];
  }

  if (isDiff) {
    return size === 2 ? ['diff-gt3'] : ['diff-le3'];
  }
  return size === 0 ? ['undiff-le2'] : ['undiff-gt2'];
}

function sizedCells(
  values: Record<string, number>,
): GastricEsdCurabilityCellId[] {
  const row = tumorFactorRow(values);
  if (!row) return [];
  return colsForHighlight(row, values.histology, values.size).map((col) =>
    gastricCurabilityCellId(row, col),
  );
}
...
export function resolveGastricEsdCurabilityHighlight(
  ...
  const baseCells = sizedCells(filled);
...
function meetsEcuraA(values: Record<string, number>, ignoreEnBlocHm = false): boolean {
  ...
  if (values.histology === 1 && values.undiffSize === 1 && values.depth === 0) return false;
  if (values.histology === 1 && values.undiffInSm === 1 && values.depth === 1) return false;
  ...
  if (values.depth === 1) {
    if (!isDifferentiatedDominant(histology)) return false;
  ...
  switch (grade) {
    case 'eCuraA':
      return [
        ...
        '【治療 GL 第7版】eCuraA には分化型優位・pT1b1（SM1）・長径 ≤3 cm も含まれます（2025年改訂）。',
      ];
    case 'eCuraB':
      return [
        ...common,
        '適応拡大切除後の根治度（第7版）。eCuraA (iii)(iv) に該当する病変で、局所再発＋初回 ESD 標本の合計 ≤30 mm などの条件を満たす場合に該当します。',
        '【治療 GL 第7版】eCuraB 後は EGD に加え、転移検索のため US または CT も望ましい。',
        '【治療 GL 第7版】H. pylori 陽性例では除菌を推奨。',
      ];
    case 'eCuraC-1':
      return [
        ...common,
        ...
        '【治療 GL 第7版】側方断端陽性・分割切除のみが eCuraA から外れる場合は eCuraC-1。追加切除は個別判断。',
      ];
    case 'eCuraC-2':
      return [
        ...common,
        ...
        '【治療 GL 第7版】eCuraC-2 は原則追加外科切除。年齢・合併症等で追加切除しない場合は LNM リスクと再発時の予後不良を説明。',
...
function ecuraAPattern(values: Record<string, number>): string {
  if (values.depth === 1) {
    return '（iv）分化型優位・長径 ≤3 cm・pT1b1（SM1）';
  }
  if (values.histology === 2) {
    return '（ii）未分化型優位・長径 ≤2 cm・pT1a・UL0';
  }
  if (values.ul === 1) {
    return '（iii）分化型優位・長径 ≤3 cm・pT1a・UL1';
  }
  return '（i）分化型優位・pT1a・UL0（長径不問）';
}
...
export function computeGastricEsdCurability(values: Record<string, number>): ScoreResult {
  ...
  if (values.histology === 1 && values.undiffInSm === 1 && values.depth === 1) {
    return buildResult('eCuraC-2', [
      '注：SM 浸潤部に未分化型成分があるため eCuraA ではなく eCuraC-2（ESD ガイドライン Fig. 6）。',
    ]);
  }

  if (meetsEcuraA(values)) {
    return buildResult('eCuraA', [
      `該当パターン：${ecuraAPattern(values)}。一括切除、HM0、VM0、Ly0、V0。`,
    ]);
  }
  ...
  if (values.depth >= 2) c2Hints.push('pT1b2（SM2）以深');
  ...
  if (values.size === 2 && values.depth === 1 && isDifferentiatedDominant(values.histology)) {
    c2Hints.push('長径 >3 cm の pT1b1（SM1）（eCuraA 条件外）');
  }
