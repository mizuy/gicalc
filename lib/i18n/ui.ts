...
export const UI: Record<Locale, UiStrings> = {
  ja: {
    ...
    about: {
      ...
      colorectum: '大腸',
      colorectumBody:
        'APCS（検診）、SPS（鋸歯状ポリープ症候群・WHO 2019 診断基準）、Vienna（消化管上皮性腫瘍の病理分類）、Paris（肉眼型）、LST（側方発育）、工藤–鶴田（pit pattern）、NICE（NBI非拡大）、WASP（HP / SSL / 腺腫）、JNET（NBI拡大）、T1 Nomogram（Kajiwara）、BBPS / Aronchick（前処置）。',
      bleeding: '出血',
  ...
  en: {
    ...
    about: {
      ...
      colorectum: 'Colorectum',
      colorectumBody:
        'APCS (screening), SPS (serrated polyposis syndrome — WHO 2019 criteria), Vienna (pathological classification of GI epithelial neoplasia), Paris (morphology), LST (laterally spreading), Kudo–Tsuruta (pit pattern), NICE (NBI, non-magnifying), WASP (HP / SSL / adenoma), JNET (NBI magnifying), T1 Nomogram (Kajiwara), BBPS / Aronchick (bowel prep).',
      bleeding: 'Bleeding',
      bleedingBody:
