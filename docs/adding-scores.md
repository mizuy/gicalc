...
ホストできる CC 図が **2つ以上**ある分類は、メインで使う最良の1ソースだけをカードに切り抜き、残りは切り抜きせず [`/atlas/{id}`](../app/atlas/[id].tsx) に載せます（並べは出版年の降順）。カタログは [`data/atlas/`](../data/atlas/) です。1ソースだけの分類に空のアトラスは作りません。フッターに図鑑タブは足さず、`/atlas` は Home 扱いにします。
...
| **画像の種類** | `figureKind` を `original` / `secondary` / `gicalc` のいずれかにする。画面の1行は Original / Not original に畳む |
| **短い出典** | `sourceShort` に切り抜き用の短い論文名（例: `Hamada 2021`）を書く。GI Calc 図は `GI Calc` |
